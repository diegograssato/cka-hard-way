...
function _delete_machine() {
    ...
    _remove_ssh_alias ${_MACHINE_NAME} >/dev/null 2>&1
...
pushd "${_MACHINE_SOURCE_PATH}" > /dev/null  
    ...
    ${_VIRSH_CMG} --print-xml > "${_MACHINE_NAME}.xml"
      if [ "$?" -ne 0 ]; then
        _warning "INFO: Fail to configure vm ${_MACHINE_NAME}!"
        exit 10;
    fi
    ...
    if [ "$?" -ne 0 ]; then
        _warning "INFO: Fail to define vm ${_MACHINE_NAME}!"
        exit 10;
    fi
    ...
    if [ "$?" -ne 0 ]; then
        _warning "INFO: Fail to generate image ${_CD_ISO_PATH}!"
        exit 10;
    fi
...
function _finalize_machine_creation() {
    ...
    _info "Done connect via ssh using [ ssh ${_MACHINE_USER}@${_MACHINE_NAME} ] and password [ ${_MACHINE_PASSWORD} ].\n" 
}
...
function _create_machine() {
   ...
   _check_machine_exists "${_MACHINE_NAME}"
    ...
    virsh start "${_MACHINE_NAME}"
    if [ "$?" -ne 0 ]; then
        _warning "INFO: Fail to start vm ${_MACHINE_NAME}!"
        exit 10;
    fi
