...
runcmd:
   ...
   - sed -i -e '/^#Port 22/s/^.*$/Port ${_MACHINE_SSH_PORT}/' /etc/ssh/sshd_config   
...
function _create_network_data2() {
  ...
  echo "Criando interface eth0 => ${_INTERNAL_BRIDGE_IP}"
  ...
  _add_ssh_alias ${_INTERNAL_BRIDGE_IP} ${_MACHINE_NAME} >/dev/null 2>&1

  if [ -n "${_EXTERNAL_BRIDGE_IP}" ]; then
    echo "Criando interface eth1 => ${_EXTERNAL_BRIDGE_IP}"
...
function _add_ssh_alias(){
 
  local _IP=${1}
  local _ALIAS=${2}
  cat << EOF | tee -a ${_SSH_HOME}
Host ${_ALIAS}
  Hostname ${_IP}
  Port ${_MACHINE_SSH_PORT}
...
function _remove_ssh_alias(){
  local _ALIAS=${1}
  sed "-i.`date +%d%m%Y-%H%M%S`" "/${_ALIAS}/,+4 d" ${_SSH_HOME}
